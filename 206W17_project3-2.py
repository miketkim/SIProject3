...
TweetsTable= 'INSERT INTO Tweets VALUES (?, ?, ?, ?, ?)'
for x in umich_tweets:
	values = (x['id_str'],x['text'], x['user']['id_str'], x['created_at'], x['retweet_count'])
	cur.execute(TweetsTable, values)
...
user_names = []
user_mentions = [x['entities']['user_mentions'] for x in umich_tweets]
for x in user_mentions:
	for y in x:
		user_names.append(y['screen_name'])


statement = 'INSERT INTO Users Values (?, ?, ?, ?)'

for x in umich_tweets:
	users = x['entities']['user_mentions']
	for y in users:
		names = api.get_user(y['screen_name'])
		tuple_= (	names['id_str'], names['screen_name'], names['favourites_count'], names['description'])
		cur.execute(statement, tuple_)
